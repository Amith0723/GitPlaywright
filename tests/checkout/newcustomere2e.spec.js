...
/**
 * Open New Sale and switch to Booking tab.
 */
...
/**
 * Generate unique dynamic customer data for testing.
 */
export function generateCustomerData(overrides = {}) {
  const rand4 = Math.floor(1000 + Math.random() * 9000);
  const randPhone = `9${Math.floor(100000000 + Math.random() * 900000000)}`;
  const timestamp = Date.now();

  return {
    fullName: overrides.fullName || `Customer_${rand4}`,
    phoneNumber: overrides.phoneNumber || randPhone,
    gender: overrides.gender || 'Male',
    email: overrides.email || `cust_${timestamp}@test.com`,
    ...overrides
  };
}

/**
 * Create a new customer via the "+ New" customer creation modal.
 */
export async function createNewCustomer(page, customerData = {}) {
  const customer = generateCustomerData(customerData);
  console.log('3. Creating New Customer with details:');
  console.log(`   • Name:   ${customer.fullName}`);
  console.log(`   • Phone:  ${customer.phoneNumber}`);
  console.log(`   • Gender: ${customer.gender}`);
  console.log(`   • Email:  ${customer.email}`);

  // Click "+ New" button near Search Customer
  const newCustomerBtn = page.locator('flt-semantics[role="button"]')
    .filter({ hasText: /^\+?\s*New$/i })
    .or(page.getByRole('button', { name: /New/i }))
    .first();

  await newCustomerBtn.waitFor({ state: 'attached', timeout: 10000 });
  await newCustomerBtn.click({ force: true });
  console.log('✅ Clicked "+ New" button');

  // Wait for Create Customer modal to load
  await page.waitForTimeout(4000);

  // 1. Phone Number input
  const phoneInput = page.locator('input[placeholder="223 665 7896"]')
    .or(page.getByRole('textbox', { name: /Phone Number|223 665 7896/i }))
    .first();
  await phoneInput.waitFor({ state: 'attached', timeout: 10000 });
  await phoneInput.click({ force: true });
  await phoneInput.fill(customer.phoneNumber);
  console.log(`   ✅ Entered Phone Number: ${customer.phoneNumber}`);
  await page.waitForTimeout(400);

  // 2. Full Name input
  const nameInput = page.locator('input[placeholder="John Doe"]')
    .or(page.getByRole('textbox', { name: /Full Name|John Doe/i }))
    .first();
  await nameInput.waitFor({ state: 'attached', timeout: 10000 });
  await nameInput.click({ force: true });
  await nameInput.fill(customer.fullName);
  console.log(`   ✅ Entered Full Name: ${customer.fullName}`);
  await page.waitForTimeout(400);

  // 3. Gender selection
  const genderBtn = page.locator('flt-semantics').filter({
    hasText: new RegExp(`^${customer.gender}$`, 'i')
  }).last();
  if (await genderBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
    await genderBtn.click({ force: true });
    console.log(`   ✅ Selected Gender: ${customer.gender}`);
    await page.waitForTimeout(400);
  }

  // 4. Email Address input (optional)
  if (customer.email) {
    const emailInput = page.locator('input[placeholder="johndoe@example.com"]')
      .or(page.getByRole('textbox', { name: /Email Address|johndoe@example.com/i }))
      .first();
    if (await emailInput.isVisible({ timeout: 2000 }).catch(() => false)) {
      await emailInput.click({ force: true });
      await emailInput.fill(customer.email);
      console.log(`   ✅ Entered Email: ${customer.email}`);
      await page.waitForTimeout(400);
    }
  }

  // 5. Submit "Add Customer"
  const addCustomerBtn = page.locator('flt-semantics[role="button"]')
    .filter({ hasText: /^Add Customer$/i })
    .or(page.getByRole('button', { name: 'Add Customer', exact: true }))
    .first();

  await addCustomerBtn.waitFor({ state: 'attached', timeout: 10000 });
  await addCustomerBtn.click({ force: true });
  console.log('✅ Clicked "Add Customer" button');

  // Wait for submission to complete and navigate back to sales screen
  await page.waitForTimeout(3500);

  // If app returned to Walk-in, switch to Booking tab to ensure full booking table
  ...
  if (await bookingTab.isVisible({ timeout: 3000 }).catch(() => false)) {
    ...
    console.log('✅ Switched to Booking tab with new customer');
    await page.waitForTimeout(1500);
  }

  // Dismiss any "Incomplete Bookings" popup if it appears
  ...
  console.log(`🎉 New customer "${customer.fullName}" successfully created and active in sale!`);
  return customer;
}

/**
 * Open Service modal and select services by keyword search or distinct indices.
 */
export async function selectServices(page, {
  ...
  if (selectedCount < maxCount) {
    console.log(`ℹ️ Selecting remaining ${maxCount - selectedCount} services from catalog...`);
    const total = await serviceCheckboxes.count();
    const candidateIndices = indices || [0, 1];
    for (const idx of candidateIndices) {
      if (selectedCount >= maxCount) break;
      if (idx < total) {
        await serviceCheckboxes.nth(idx).click({ force: true });
        selectedCount++;
        console.log(`   ✅ Selected service at index #${idx}`);
...
/**
 * Click Quantity (+) for any service row using the Qty column header anchor.
 */
...
export async function proceedToCheckout(page) {
  ...
  await page.screenshot({ path: 'newcustomer-checkout-landed.png', fullPage: true });
  console.log('📸 Screenshot saved: newcustomer-checkout-landed.png');
}
...
/**
 * End-to-end New Customer Creation & Checkout Workflow.
 */
export async function runNewCustomerE2EWorkflow(page, config = {}) {
  const {
    loginCredentials = {},
    customerData = {},
    services = [{ search: 'Hair' }, { search: 'Trim' }],
  ...
  console.log('════════════════════════════════════════════════════════════════');
  console.log('🚀 Executing New Customer Creation & Checkout E2E Workflow');
  console.log('════════════════════════════════════════════════════════════════');

  // 1. Login
  await login(page, loginCredentials);

  // 2. Open New Sale & switch to Booking tab
  await openNewSaleBooking(page);

  // 3. Create a Brand New Customer via "+ New"
  const createdCustomer = await createNewCustomer(page, customerData);
...
test('New Customer E2E: Create Customer -> Add Services -> Staff -> Checkout -> Pay Later -> Complete Sale', async ({ page }) => {

  const workflowConfig = {
    customerData: {
      // Dynamic random customer data will be automatically generated:
      // - Unique Phone Number (e.g. 9XXXXXXXXX)
      // - Unique Name (e.g. Customer_XXXX)
      // - Gender (Male)
      // - Unique Email
    },
    services: [
    ...
    deleteRowIndex: 1, // Delete 2nd service row
    maxPopups: 2,
    closeBrowser: true
  };

  await runNewCustomerE2EWorkflow(page, workflowConfig);
});
